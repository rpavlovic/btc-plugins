			...
			<?php if ($this->GetAllSavedSearch()): ?>
				<select style="width: 130px; vertical-align: top;" id="save-search" onchange="top.location = this.value">
					...
					<?php foreach ($this->GetAllSavedSearch() as $value): ?>
						<option value="?page=<?php echo $this->MenuID?>&wl=members&<?php echo http_build_query($value['value'])?>" <?php if (wlm_arrval($_GET, 'saved_search') == $value['name']) echo " selected='true'"; ?>><?php echo $value['name'] ?></option>
					<?php endforeach; ?>
				</select>
			<?php endif; ?>
		</p>
