 ...
 * Version: $Id: integration.shoppingcart.stripe.php 2462 2014-11-17 17:49:17Z mike $
 */
$__index__ = 'stripe';
...
if (wlm_arrval($_GET, 'cart') == $__index__) {
   ...
   ?>
		...
		<form method="post" id="stripe_form">
			...
			<p><?php _e('Get your API Key from ', 'wishlist-member'); ?><a href="https://dashboard.stripe.com/account/apikeys" target="_blank">https://dashboard.stripe.com/account/apikeys</a></p>
		...
		<?php $prorate = empty($stripesettings['prorate']) ? "yes" : $stripesettings['prorate']; ?>
			...
			<h2 class="wlm-integration-steps">Web Hook</h2>
			<p><?php _e('Add this URL in ', 'wishlist-member'); ?> <a href="https://dashboard.stripe.com/account/webhooks" target="_blank">Stripe Webhooks</a></p>
			<p>
				<input readonly="readonly" style="width: 60%;padding:4px; color:#1A1A1A;" type="text" value="<?php echo $stripethankyou_url ?>?stripe_action=sync" />
			</p>
			<p>&nbsp;</p>
			<p><a href="#" class="button-secondary" onclick="javascript:jQuery('#stripe_form')[0].reset();
					return false;">Reset All Settings</a> <input type="submit" class="button-primary" value="Update All Settings"/></p>
