 ...
 * Version: $Id: integration.shoppingcart.clickbank.php 2462 2014-11-17 17:49:17Z mike $
 */
...
if (wlm_arrval($_GET, 'cart') == $__index__) {
	if (!$__INTERFACE__) {
		...
		if (wlm_arrval($_POST, 'cbproducts')) {
			$this->SaveOption('cbproducts', $_POST['cbproducts']);
		...
		<blockquote>
			<form method="post">
				<h2 class="wlm-integration-steps"><?php _e('Step 1: ClickBank Account Information', 'wishlist-member'); ?></h2>
			...
			<form method="post">
				<h2 class="wlm-integration-steps"><?php _e('Step 2: Use the URL below for your ClickBank products "Thank You Page" and "Instant Notification URL":', 'wishlist-member'); ?></h2>
			...
			?>
			<form method="post">
				<h2 class="wlm-integration-steps"><?php _e('Step 3: Specify the ClickBank Product IDs that you wish to use for each of your Membership Levels', 'wishlist-member'); ?></h2>
				<table class="widefat">
					...
					<thead>
						...
						<tbody>
							...
							<tr class="<?php echo $alt++ % 2 ? '' : 'alternate'; ?>" id="wpm_level_row_<?php echo $sku ?>">
								<td><b><span><?php echo stripslashes($level['name']) ?></span></b></td>
								<td>
									<?php foreach ((array)$product_ids as $key => $product_id):?>
										<input type="text" name="cbproducts[<?php echo $sku; ?>][]" value="<?php echo strtolower($product_id); ?>" size="16" style="text-align:center"><br/>
										<?php endforeach; ?>
								...
								<td><?php foreach ((array)$product_ids as $key => $product_id):?>
									<div><span id="cb_pay_link-<?php echo $sku . '_' . $product_id; ?>"></span></div><br>
								<?php endforeach; ?>
								</td>
								<?php
								endforeach;
