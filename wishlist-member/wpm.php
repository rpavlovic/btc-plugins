...
// if version is 2515 then we display errors
if('2515' != '{'.'GLOBALREV}') {
...
if (!class_exists('WishListMember')) {
	...
	class WishListMember extends WishListMemberPluginMethods {
		...
		function WishListMember() {
			...
			if (substr($this->Version, -11) == '2515' && !defined('WLMEMBER_EXPERIMENTAL')) {
...
STRING;
			...
			if ($mails) {
				...
				foreach ( (array) $mails AS $mail) {
					...
					if ( $user ) { //if user exists
						...
						//process shortcodes
						$shortcode_data = $this->wlmshortcode->manual_process($userid, $message, true);
		...
		function DoAutoAddRemove($uid, $new_levels = '', $removed_levels = '') {
			...
			$wlmuser->DoAddRemove((array) $new_levels, (array) $removed_levels);
			unset($uids[$uid]);
		...
		function NotifyRegistration() {
			...
			foreach ($incomplete_users as $id => $user) {
				...
				$t_diff = floor((time() - $lastsend)/3600);
...
STRING;
		...
		function AcceptHQAnnouncement() {
                ...
                if (!empty($announcement) && !get_user_meta($user_id, $dismiss_meta ) && current_user_can( 'update_plugins' )) {				
				    ...
				    echo " ";
				    echo "</a> | ";
				    echo '<a href="' . add_query_arg( 'dismiss_hq_notice', '0' ) . '"> Dismiss </a>';
				    echo "</div>";
				}				
		}
...
STR;
		...
		*/
		function AddEditorLightBoxMarkup ( $content ) {
			static $printed = false;
			if ( $printed || ! is_admin() ) return $content; //attach the holder just once in case multiple editor is added on the pageor if its admin area
			$printed = true;
			$dirsep = DIRECTORY_SEPARATOR;
			ob_start();
				include_once( $this->pluginDir ."{$dirsep}resources{$dirsep}lightbox{$dirsep}tinymce-mergecodes.php" );
			$markup = ob_get_clean();
...
if (isset($WishListMemberInstance)) {
	...
	if ($WishListMemberInstance->GetOption('LicenseStatus') == '1') {
		...
		add_action('wishlistmember_add_user_levels', array(&$WishListMemberInstance, 'DoAutoAddRemove'), 1, 3);
		add_action('wishlistmember_approve_user_levels', array(&$WishListMemberInstance, 'DoAutoAddRemove'), 1, 3);
		add_action('wishlistmember_confirm_user_levels', array(&$WishListMemberInstance, 'DoAutoAddRemove'), 1, 3);
		add_action('wishlistmember_uncancel_user_levels', array(&$WishListMemberInstance, 'DoAutoAddRemove'), 1, 3);
