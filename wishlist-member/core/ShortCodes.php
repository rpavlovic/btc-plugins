 ...
 * @version $Rev: 2479 $
 * $LastChangedBy: mike $
 * $LastChangedDate: 2014-12-05 22:29:04 -0500 (Fri, 05 Dec 2014) $
 */
...
//				$shortcodes[] = 'wlmprivate_' . $level['name'];
    ...
    function reverse_private_tags($atts, $content, $code) {
		...
		//lets check if one of levels in the tag is not in users level
		$users_not_level = array_diff( $tag_levels, $user_levels );
		if( count( $users_not_level ) > 0 ) { //if theres a level in the tag that users dont have
			//return  the content
            return do_shortcode($content);
		} else { //if users has all levels in the tag, display the message
