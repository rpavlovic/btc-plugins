		...
		function DoAddRemove($new_levels, $removed_levels) {
			...
			foreach($new_levels AS $level) {
				if($this->Levels[$level]->Active) {
					$remove = array_merge($remove, array_keys((array) $wpm_levels[$level]['removeFromLevel']));
					$add = array_merge($add, array_keys((array) $wpm_levels[$level]['addToLevel']));
				}
			}
			// we don't remove the newly added levels
			$remove = array_diff($remove, $new_levels);

			// we don't add the newly removed levels
			$add = array_diff($add, $removed_levels);
			...
			$levels = array_unique(array_diff(array_merge($current_levels, $add), $remove));

			// we update the levels
			$this->WL->SetMembershipLevels($this->ID, $levels);
