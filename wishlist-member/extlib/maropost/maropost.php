	...
	public function request($method, $action, $params = array(), $data = array()) {
		...
		switch ($method) {
			...
			case 'POST':
				...
				$resp = wp_remote_post( $gateway, array('sslverify' => false, 'timeout' => 5, 'body' => $body, 'headers' => $headers));
				if(is_wp_error($resp)) {
					throw new Exception($resp->get_error_message());
				}
				$resp = $resp['body'];
			...
			default:
				...
				$resp = wp_remote_request( $gateway,
					array('method' => $method, 'timeout' => 5, 'body' => $body, 'headers' => $headers, 'body' => $body)
				);
