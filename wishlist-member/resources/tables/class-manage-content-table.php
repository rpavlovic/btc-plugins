...
class WLM_Manage_Content_Table extends WP_List_Table {
	...
	function column_default( $item, $column_name ) {
		...
		case 'wlm_levels':
			...
			$immutable    = implode( ',', $item['wlm_levels_immutable'] );
	...
	function column_post_title( $item ) {
		$actions = array(
			...
			'view' => sprintf( '<a href="%1$s" target="_blank" class="wlm-view" id="wlm-view-%2$d">%3$s</a>', get_permalink($item['ID']), $item['ID'], __( 'View', 'wishlist-member' ) ),
		);
