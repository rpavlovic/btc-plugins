...
$LastChangedDate: 2015-06-10 19:43:38 -0700 (Wed, 10 Jun 2015) $
$Rev: 12995 $
*/
...
function sp_LogInOutButton($args='', $inLabel='', $outLabel='', $toolTip='') {
	...
	if (is_user_logged_in() == true) {
		if (!$mobileMenu) {
			$out.= "href='$logOutLink'>";
			if (!empty($logOutIcon)) $out.= sp_paint_icon($iconClass, SPTHEMEICONSURL, $logOutIcon);
		...
		if (!$mobileMenu) {
			...
			if (!empty($logInIcon)) $out.= sp_paint_icon($iconClass, SPTHEMEICONSURL, $logInIcon);
