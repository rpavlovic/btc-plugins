...
$LastChangedDate: 2015-02-13 13:41:03 -0800 (Fri, 13 Feb 2015) $
$Rev: 12476 $
*/
...
function sp_ProfileShowFacebook($args='', $label='') {
	...
	$the_url = get_the_author_meta( 'facebook', $spProfileUser->ID );

	if ($get) return $the_url;

	# output first name
	if (!empty($the_url) || $showEmpty) {
		$out = '';
		$out.= "<div class='$leftClass'>";
		$out.= "<p class='$tagClass'>$label:</p>";
		$out.= '</div>';
		...
		if (empty($the_url)) {
			...
			$url = sp_filter_display_links($the_url);
			$spFilters = sp_get_option('sffilters');
			if ($spFilters['sfnofollow']) $url = sp_filter_save_nofollow($url);
			if ($spFilters['sftarget']) $url = sp_filter_save_target($url);
		}
...
function sp_ProfileShowTwitter($args='', $label='') {
	...
	$the_url = get_the_author_meta( 'twitter', $spProfileUser->ID );
...
function sp_ProfileShowInstagram($args='', $label='') {
	...
	$the_url = get_the_author_meta( 'instagram', $spProfileUser->ID );
