...
$LastChangedDate: 2015-05-18 02:48:27 -0700 (Mon, 18 May 2015) $
$Rev: 12905 $
*/
...
function sp_TopicIndexPageLinks($args='', $label='', $toolTip='', $jumpToolTip='') {
	...
	# sanitize before use
	$tagClass		= esc_attr($tagClass);
	if (!empty($prevIcon)) $prevIcon	= sp_paint_file_icon(SPTHEMEICONSURL, sanitize_file_name($prevIcon));
	if (!empty($nextIcon)) $nextIcon	= sp_paint_file_icon(SPTHEMEICONSURL, sanitize_file_name($nextIcon));
	if (!empty($jumpIcon)) $jumpIcon	= sp_paint_file_icon(SPTHEMEICONSURL, sanitize_file_name($jumpIcon));
	$iconClass		= esc_attr($iconClass);
	$pageLinkClass	= esc_attr($pageLinkClass);
