    	...
    	new AjaxUpload(button,{
    		...
    		onComplete: function(file, response){
    			...
    			if (response==="success"){
    				...
    				jQuery('<table style="width:100%"></table>').appendTo('#sf-smiley-imgs').html('<tr><td style="width:5%;text-align:center"><img class="spSmiley" src="<?php echo SFSMILEYS; ?>' + file + '" alt="" /></td><td class="sflabel" style="text-align:center;width:30%"><input type="hidden" name="smfile[]" value="' + file + '" />' + file + '</td><td style="width:30%;text-align:center"><input type="text" class="sfpostcontrol" size="20" name="smname[]" value="" /></td><td style="width:25%;text-align:center"><input type="text" class="sfpostcontrol" size="20" name="smcode[]" value="" /></td><td style="width:5%;text-align:center" class="sflabel"><input class="sfnewradio'+scount+'" type="checkbox" name="sminuse-new-'+scount+'" id="sf-new-'+scount+'" checked="checked"/><label for="sf-new-'+scount+'"></label></td><td class="sflabel" style="text-align:center;width:5%"><img src="<?php echo SFCOMMONIMAGES; ?>' + 'delete.png' + '" title="<?php echo esc_js(spa_text('Delete Smiley')); ?>" alt="" onclick="spjDelRowReload(\'' + site + '\', \'sfreloadsm\');var x=jQuery(\'#smiley-count\').val();jQuery(\'#smiley-count\').val(parseInt(x)- 1);" /></td></tr>');
