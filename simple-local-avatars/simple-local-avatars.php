...
class Simple_Local_Avatars {
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = '' ) {
		...
		if ( false && ! empty( $avatar_rating ) && 'G' != $avatar_rating && ( $site_rating = get_option( 'avatar_rating' ) ) ) {
	 ...
	 * @param object $profileuser User object
	 */
	public function btc_edit_user_avatar( $profileuser ) {
?>
							<div class="row">
								<label for="simple-local-avatar"><?php _e('Upload Headshot','simple-local-avatars'); ?></label>
								...
								?>
										...
										<p>
											<?php if ( current_user_can( 'upload_files' ) && did_action( 'wp_enqueue_media' ) ) : ?><a href="#" class="button hide-if-no-js" id="simple-local-avatar-media"><?php _e( 'Choose from Media Library', 'simple-local-avatars' ); ?></a> &nbsp;<?php endif; ?>
											<?php /* <a href="<?php echo $remove_url; ?>" class="button item-delete submitdelete deletion" id="simple-local-avatar-remove"<?php if ( empty( $profileuser->simple_local_avatar ) ) echo ' style="display:none;"'; ?>><?php _e('Delete local avatar','simple-local-avatars'); ?></a> */ ?>
											<input type="button" class="button item-delete submitdelete deletion"<?php if ( empty( $profileuser->simple_local_avatar ) ): echo ' style="display:none;"'; else: ?> onclick="location='<?php echo $remove_url; ?>'"<?php endif; ?> value="Delete Headshot">
										</p>
								<?php
									} else {
										if ( empty( $profileuser->simple_local_avatar ) )
							...
							<?php
								...
								echo "\n\t<input type='hidden' name='simple_local_avatar_rating' value='PG' " . checked( $profileuser->simple_local_avatar_rating, $key, false ) . "/>";
							?>
